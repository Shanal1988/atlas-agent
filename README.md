...
Atlas is a command-line equity research pipeline that takes a company name and runs it through a multi-stage analytical framework, producing a scored investment thesis with a position size recommendation.

It is built for long-term, value-growth investors who want a disciplined, repeatable framework — not a trading signal. Inspired by the philosophies of **Warren Buffett**, **Charlie Munger**, **Philip Fisher**, **Peter Lynch**, **Mohnish Pabrai**, and **Adam Seessel** (*Where the Money Is: Value Investing in the Digital Age*).
...
Atlas resolves the company ticker, fetches financials, and runs a full multi-stage analysis, streaming progress to the terminal. At the end it saves a structured thesis to `data/theses/`.
...
|-------|-------|-------------|
| 1 | **Discovery** | Resolves ticker (LLM + Yahoo Finance fallback), fetches financials from FMP + yfinance, computes Enterprise Value and net cash |
| 2 | **Industry Analysis** | Analyzes TAM, market structure, competitive positioning, peer group |
| 3 | **Munger Four Filters** | Charlie Munger's F1 Understandability / F2 Moat / F3 Management / F4 Price filters |
| 4 | **BMP Gate** | 5-question Business / Moat / Price checklist — YES / PARTIAL / NO via LLM |
| 5 | **Fisher Analysis** | Philip Fisher 15-point quality analysis via LLM + Tavily web research |
| 6 | **Stock Selection** | 8-question checklist including Buffett Dollar Test, Maintenance vs. Growth Capex separation, and FCF quality flags |
| 7 | **Process Scoring** | Ten Vital Signs, Feroldi Quality Score Screen, Investment Lifecycle Stage (1–10), Lynch Classification |
| 8 | **Valuation** | Multi-model intrinsic value: Dhandho, Ben Graham, DCF, Expected Returns |
| 9 | **Risk Scoring** | Diamond-to-Egg crushability framework — counts NOs, applies risk penalties, outputs position size |
| 10 | **Thesis Writer** | Structured investment thesis: executive summary, bull/bear case, watch points, 2–5–10 year outlook |
| 11 | **LLM-as-a-Judge** | Independent audit layer — score justification checks, bull/bear balance, cross-stage consistency |
...
## Adam Seessel: Operating Earnings Yield (OEY) Framework

Atlas implements the digital value investing methodology from Adam Seessel's *Where the Money Is: Value Investing in the Digital Age* to avoid false price vetoes on secular tech compounders.

**Key principles:**

- **Enterprise Value basis**: Denominator uses $EV = \text{Market Cap} + \text{Debt} - \text{Cash}$ so cash-rich balance sheets (e.g. Alphabet's $121B+ net cash) are not penalized.
- **Normalized steady-state EBIT**: GAAP forces digital companies to expense long-term R&D and customer acquisition S&M immediately, suppressing reported margins. Atlas normalizes using segment-level steady-state margins:
  - Alphabet (GOOGL/GOOG): ~38.4% blended (Search 46%, Cloud 34%, YouTube 38%)
  - Meta (META): ~48.5%
  - Amazon (AMZN): ~15.3% blended (Retail, AWS, Ads)
  - Microsoft (MSFT): ~39.8%
  - SaaS / Digital baseline: 32–36%
- **Dynamic growth-adjusted hurdle rate**:
  - **Elite Compounders** (Revenue CAGR ≥ 12% or ROE ≥ 20%): OEY hurdle of **≥ 3.5%** (~28× EV/NOPAT)
  - **Mature / Slower growers**: Standard **≥ 5.0%** hurdle applies

---

## Maintenance vs. Growth Capex Separation

Stock Selection Q7 now separates **Maintenance Capex** (required to sustain existing revenues) from **Growth Capex** (discretionary investment in new capacity). If companies do not disclose maintenance capex explicitly, Atlas derives an estimate using industry-standard depreciation ratios so high-reinvestment compounders are not mislabelled as capital-intensive.
...
# LLM providers — Atlas cascades through these automatically
...
| [Groq](https://console.groq.com) | Yes (100k tokens/day) | Primary LLM scoring and writing |
| [Gemini](https://ai.google.dev) | Yes | LLM fallback (rate limit resilience) |
| [OpenRouter](https://openrouter.ai) | Yes (free models) | LLM fallback of last resort |

> **Multi-provider LLM resilience:** Atlas cascades through Groq → Gemini → OpenRouter on rate limits (HTTP 429) or failures. Reasoning model tags (`<think>...</think>`) are automatically stripped. Each run uses approximately 15–20k tokens.
...
├── agents/
│   ├── context.py               # Shared OEY engine (Adam Seessel framework)
│   ├── discovery.py             # Stage 1: ticker resolution + financials + EV
│   ├── industry.py              # Stage 2: industry & competitor analysis
│   ├── munger.py                # Stage 3: Munger Four Filters
│   ├── bmp_gate.py              # Stage 4: BMP checklist
│   ├── fisher.py                # Stage 5: Fisher 15-point analysis
│   ├── stock_selection.py       # Stage 6: selection checklist + Capex separation
│   ├── process_scoring.py       # Stage 7: Vital Signs, Quality Screen, Stage, Lynch
│   ├── valuation.py             # Stage 8: multi-model intrinsic value engine
│   ├── risk_scoring.py          # Stage 9: Diamond-to-Egg risk + position sizing
│   ├── thesis_writer.py         # Stage 10: investment thesis generation
│   ├── judge.py                 # Stage 11: LLM-as-a-Judge audits
│   ├── llm_client.py            # Multi-provider LLM failover + think-tag cleaner
│   ├── fmp_client.py            # FMP API wrapper with 429 retry logic
│   └── web_search.py            # Tavily web search with multi-engine fallback
├── data/
│   └── theses/                  # Saved thesis files (gitignored)
├── evals/                       # Evaluation suite (see Roadmap below)
...
- **LLM scoring** — Multi-provider cascade: Groq → Gemini → OpenRouter
...
| Alphabet (GOOGL) | 2.0/5 | 14.0/15 | — | Glass Bottle | WATCHLIST |
...
- LLM rate limits may cause some scoring stages to fall back to shorter model responses; LLM-as-a-Judge flags material quality drops.
...
After `_resolve_ticker()` in `discovery.py`, fetch `info["quoteType"]` from yfinance. If the type is not an investable equity, print a clear error and call `sys.exit(1)`. Pipeline stops immediately — no data is fetched, no LLM tokens are used.
...
After each scoring agent parses its LLM response, check whether the output is suspiciously uniform — all YES or all NO is a strong sign of hallucination or prompt collapse.
...
Runs all LLM scoring calls N times on the same company and measures variance. Discovery is run once and cached to avoid repeated API calls. Tavily research for Fisher is also cached — variance should come from LLM only, not from different web results.
...
SUITE = {
    ...
    "GOOGL":    {"bmp": (1.5, 3.5), "fisher": (12.0, 15.0), "decision": {"WATCHLIST"}},
}
```

```
Prompt Regression Suite -- 2026-09-08
---------------------------------------
...
GOOGL       2.0 PASS  14.0 PASS  --  PASS WATCHLIST PASS
```
...
`prepare_scoring_data.py` reads all `data/theses/*.json` and reconstructs message-format training pairs for BMP, Selection, and Risk. The `assistant` turn is rebuilt from the saved `answers`/`factors` arrays — no new LLM calls needed.
...
**Model:** Same multi-provider LLM cascade. No additional API key. Adds ~900–1100 tokens per full pipeline run across all four judges.
...
#### Judge 1 — Score Justification Auditor (LLM)

**Where:** `bmp_gate.py`, `fisher.py`, `stock_selection.py` — called after parsing, before printing.

Checks whether the reasoning for each extreme score (top 3 highest and bottom 3 lowest per stage) is consistent with the company data provided. A second LLM pass on the flagged items only — not a full re-score.
...
- Risk L2 scored NO (diversified buyer base) with no supporting evidence in context — triggers **Negative Justification Audit**
...
#### Judge 2 — Bull/Bear Balance Check (LLM)
...
#### Judge 3 — Thesis Coherence Check (rules + conditional LLM)
...
If no rule triggers, a brief LLM pass checks whether the decision narrative is logically consistent with the position size and conviction stated in the scores.
...
Pure arithmetic on existing result dicts — no LLM call.
